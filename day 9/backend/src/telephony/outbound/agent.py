...
from database_calls import record_call
...
def build_greeting(customer_name: str, reason: str, metadata: dict) -> str:
    ...
    return greetings.get(reason, greetings["customer_callback"]).get(
        "hi", greetings["customer_callback"]["en"]
    )
...
class OutboundAgent(Agent):
    ...
    def __init__(
        self, ctx: JobContext, customer_name: str, reason: str, metadata: dict
    ) -> None:
    ...
    def on_session_close(*args):
        try:
            ...
            if order_placed:
                ...
                # For outbound calls (e.g. order confirmation, payment reminder),
                # success might not always mean "order placed", but for the MVP dashboard
                # we'll track if they placed an order, or just mark it failed for now
                # unless they placed an order.
                outcome = "failed"
                failure_reason = "no_order"

            call_id = f"CALL-{ctx.room.name}-{int(call_start_time)}"

            # The customer phone is our user identifier in outbound calls
            record_call(
                call_id=call_id,
            ...
            logger.info(
                "Outbound call analytics recorded: %s outcome=%s duration=%ds",
                call_id,
                outcome,
                duration,
            )
        except Exception as exc:
            logger.error("Failed to record outbound call analytics: %s", exc)
