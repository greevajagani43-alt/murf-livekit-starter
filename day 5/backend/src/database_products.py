...
def get_product_stock(product_name: str) -> Optional[int]:
    """Return the current stock quantity for a given product name."""
    with _get_conn() as conn:
        row = conn.execute(
            "SELECT qty FROM products WHERE LOWER(name) = LOWER(?)",
            (product_name,),
        ).fetchone()
