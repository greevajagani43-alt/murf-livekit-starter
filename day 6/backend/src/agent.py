"""
agent.py  —  Saathi Outbound Agent (Ratan Kirana Store)
────────────────────────────────────────────────────────
Day 6: The agent now makes OUTBOUND calls to customers' phones
via Twilio SIP Trunk ↔ LiveKit SIP.
...
class Assistant(Agent):
    """Saathi — Ratan Kirana outbound voice assistant."""
...
async def my_agent(ctx: JobContext):
    ...
    logger.info("Starting session  room=%s  user_id=%s  outbound=%s",
