...
──────────
    ...
    get_product_stock(product_name) → list[dict]  (LIKE search, full product rows)
"""
...
def get_product_stock(product_name: str) -> list[dict]:
    """Return full product rows matching the given product name (partial LIKE search).

    Returns a list of dicts with keys: id, name, size, price, icon, category, qty.
    Returns an empty list if no product is found.

    This is intentionally a LIKE/partial search so callers like check_stock()
    can find "Aashirvaad Atta" even when the user says just "atta".
    """
